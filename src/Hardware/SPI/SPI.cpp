...
void SPI::errorInterrupt() {
	if(HAL_SPI_GetError(_pHandler) > HAL_SPI_ERROR_NONE) { operationState = FINISH; }
}
...
void SPI::transmitThenReceive(
	...
	// operation.callback_f = [&, CSPort, CSPin, pData_rx, rxSize](uint8_t *data, uint16_t size) {
	// 	receive(CSPort, CSPin, pData_rx, rxSize);
	// };
	operations.push(operation);
